...
!
        ...
        real, dimension(nx, ny, nz, 4) :: bb_pg  ! dat field
          ...
          do m = 1, ny
            do l = 1, nx
              if ((l > nx + nx*ipx + nghost .or. m > ny + ny*ipy + nghost .or. n > nz + nz*ipz + nghost .or. &
                ...
                f(l + nghost - nx*ipx, m + nghost - ny*ipy, n + nghost - nz*ipz, iax:iaz) = bb_pg(l, m, n, 1:3)
              end if
            end do
          end do
        end do
...
! real, dimension(mx, my, mz, mfarray) :: f
        ...
        write (*, *) 'bb_pg', size(bb_pg, dim=1), size(bb_pg, dim=2), &
            size(bb_pg, dim=3), size(bb_pg, dim=4)
